...
Shader "taecg/ShaderToy/test2"
{
	SubShader
	{
		Pass
		{
			...
			{
				// float3 color = lerp(fixed3(0.3,0.1,0.3),fixed3(0.1,0.4,0.5),i.uv.y);	
				float3 color = 0.5 + 0.5*cos(_Time.y*0.5 + i.uv.xyx + float3(0, 2, 4));

				float2 p = 2.0*(i.uv*_ScreenParams.xy)-_ScreenParams.xy;//四方格子
				...
				//坐标按屏幕像素进行偏移
				p = 2.0* (i.uv *_ScreenParams.xy) - _ScreenParams.xy;
				float time = _Time.x;
				float t= 0.2 + time * 0.3;
