// URP版(需手动找开启UniversalRP中的OpaqueTexture)，默认管线的在更下面：
...

{
  Properties
  {
    ...
    [Toggle] _CheapNormals("Use Cheap Normals", Float) = 0
      ...
      _DropScale("Drop Scale", Range(0.5,2)) = 1
...
{
    // 获取原始图片的 Alpha（水洼图片的透明信息）
    float4 original = tex2D(_MainTex, IN.uv);
    float alpha = original.a * IN.color.a;  // 结合顶点颜色和材质 Tint

    // 如果完全透明，直接丢弃（提高性能，避免不必要的计算）
    if (alpha < 0.01) discard;

    // ---------- 雨滴效果计算（原 Shader 逻辑，未修改） ----------
    float2 iRes = _ScreenParams.xy;
    ...
    float2 sampleUV = UV + float2(n.x, -n.y);
    ...
    float dropMask = 1; // 保持原有逻辑
    if (_UseSpecular > 0.5 && dropMask > 0.001)
    {
    ...
    // 最终输出：颜色使用雨滴效果，透明度使用水洼图片的 alpha
    float4 outCol = float4(col, alpha);

    // 应用 UI 剪裁（保留原 Shader 的剪裁逻辑）
